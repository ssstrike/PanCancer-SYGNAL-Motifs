    ...
    for set1 in range(len(geneSets)):
        ...
        for cohort in [0]:#,1,2]:
            ...
            if len([i for i in binExp_lgg[cohorts[cohort]].index if i in model_and.states[0].keys()])==3:
                ...
                grades = list(pd.Series(list(set(list(pheno_lgg['TCGA']['subtype'])))).dropna())
...
#    binIndex[count]=newIndex[count]
    count = count+1
#binExp_lgg['TCGA'] = pd.read_csv('ACCBinarize.csv', header=0, index_col=0)#read written binexp csv
binExp_lgg['TCGA'].index=newIndex

# Phenotypes
pheno_lgg = {}
pheno_lgg['TCGA'] = pd.read_csv('phenotypes_panCancer.csv', header=0, index_col=0)
pheno_lgg['TCGA'] = pheno_lgg['TCGA'].loc[pheno_lgg['TCGA']['tumor']==args.tumors]
