...
#        print header1
        ...
        for cohort in range(len(binExp_lgg)):#range(len(cohorts)):::range(len(runFile)):::len(binExp_lgg)
            ...
            if len([i for i in binExp_lgg[cohorts[cohort]].index if i in model_and.states[0].keys()])==3:
                ...
                print len(groups)
                pw1 = pairwise_logrank_test(event_durations=T, event_observed=E, groups = groups)#here, divide by zero:Issue probably with groups==0 should be something >0
                #print pw1.keys()
                # Add to file
                ...
                print 'attrDict1'
                print attrDict
                print 'attrSets1 Previous'
                print attrSets
                rmvList = []
                for i in attrSets:# removes filtered item from corrosponding list, attrSets.
                    if i not in tmp1:
                        rmvList.append(i)
                        #attrSets.remove(i)# not removing all items, but why, it should work.
                for i in rmvList:
                    attrSets.remove(i)#fixed it here.
                print 'attrSets1 After'
                print attrSets
                # Heatmap
                ax = plt.subplot(inner[0])
                ...
                for i in range(1):
                    for j in range(len(attrSets)):#quick fix to run code, added -1, due to out of index:::attrSets
                        if props1[i,j]>0.7:
                            col1 = 'w'
                        else:
                            col1 = 'k'
...
# Read in binarized data args.tumors (only pyBin if file not found)
binExp_lgg = {}
#binExp_lgg['TCGA'] = pd.read_csv('exprs_all/'+'MESO'+'_RNAseq.csv', header=0, index_col=0)#.transpose()
...
#    count = count+1
#binExp_lgg['TCGA'] = pd.read_csv('MESO_TCGA_Binarize.csv', header=0, index_col=0)#MESO_GSE_29354_Binarize
#binExp_lgg['TCGA'].index=newIndex

#binExp_lgg['TCGA'].to_csv('MESO_TCGA_Binarize.csv', index=False)
phenoList = ['BUENO','TCGA','GSE']# Use only expr data from these sources.
...
for numExpFile in range(len(runFile)):
    ...
    #binExp_lgg[runName]=binExp_lgg[runName].dropna()
    if not os.path.isfile('Reconstructed_exprs/'+'Bin'+args.tumors+'_'+runFile[numExpFile].split('_')[0]+'_'+runFile[numExpFile].split('_')[1]+'_Binarize.csv'):
    ...
    for geneid in binIndex:
        if entrez2symbol.has_key(str(geneid)):
            ...
            newIndex.append(geneid)
        count = count+1
    if os.path.isfile('Reconstructed_exprs/'+'Bin'+args.tumors+'_'+runFile[numExpFile].split('_')[0]+'_'+runFile[numExpFile].split('_')[1]+'_Binarize.csv'):
        binExp_lgg[runName] = pd.read_csv('Reconstructed_exprs/'+'Bin'+args.tumors+'_'+runFile[numExpFile].split('_')[0]+'_'+runFile[numExpFile].split('_')[1]+'_Binarize.csv', header=0, index_col=0)
    else:
        print 'Error Bin file not created'
    binExp_lgg[runName].index=newIndex
    if not os.path.isfile('Reconstructed_exprs/'+'Bin'+args.tumors+'_'+runFile[numExpFile].split('_')[0]+'_'+runFile[numExpFile].split('_')[1]+'_Binarize.csv'): 
        binExp_lgg[runName].to_csv('Reconstructed_exprs/'+'Bin'+args.tumors+'_'+runFile[numExpFile].split('_')[0]+'_'+runFile[numExpFile].split('_')[1]+'_Binarize.csv', index=False)
...
# Phenotypes
pheno_lgg = {}
pheno_lgg['BUENO'] = pd.read_csv('Phenotype/phenotypes_meso_noFilter_Bueno.csv', header=0, index_col=0)#:::phenotypes_meso_noFilter_Bueno
pheno_lgg['BUENO'] = pheno_lgg['BUENO'].loc[pheno_lgg['BUENO']['tumor']==args.tumors]#args.tumors
pheno_lgg['BUENO'] = pheno_lgg['BUENO'][["tumor","OS.time","OS","subtype"]]

pheno_lgg['GSE'] = pd.read_csv('Phenotype/GSEPheno_panCancer.csv', header=0, index_col=0)#GSE29354_pData_reconMESO:::GSEPheno_panCancer
pheno_lgg['GSE'] = pheno_lgg['GSE'].loc[pheno_lgg['GSE']['tumor']==args.tumors]
pheno_lgg['GSE'] = pheno_lgg['GSE'][["tumor","OS.time","OS","subtype"]]

pheno_lgg['TCGA'] = pd.read_csv('Phenotype/phenotypes_panCancer.csv', header=0, index_col=0)#phenotypes_mesoTCGA:::phenotypes_panCancer
pheno_lgg['TCGA'] = pheno_lgg['TCGA'].loc[pheno_lgg['TCGA']['tumor']==args.tumors]#args.tumors
pheno_lgg['TCGA'] = pheno_lgg['TCGA'][["tumor","OS.time","OS","subtype"]]

#pheno_lgg['TCGA'] = pheno_lgg[pheno_lgg.keys()[1]][["tumor","OS.time","OS","subtype"]]
